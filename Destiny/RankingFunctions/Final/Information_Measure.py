...
class Information_Measure(Mesure):
    ...
    def __init__(self):
        super().__init__()
        self._liste_mesures = Information_Measure.mesures_existantes
        self.__entropy = {}
    ...
    def fit(self,data,target):
        ...
        for i in d:
            ...
            self.__entropy[1][str(cpt)] = Information_Measure.h(L)
            cpt = cpt +1
        L = []
        L.append (target)
        self._attributs["-1"] = target
        self.__entropy[1]["-1"] = Information_Measure.h(L)
    ...
    def getEntropy(self,liste_nums):
        ...
        if not tuple(liste_nums) in self.__entropy[len(liste_nums)].keys():
            ...
            self.__entropy[len(liste_nums)][tuple(liste_nums)] = Information_Measure.h(L)
