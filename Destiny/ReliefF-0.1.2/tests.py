"""
    Unit tests for RankingFunctions.
"""
...
def test_transform():
    """Make sure RankingFunctions transforms correctly"""
    data = pd.read_csv('data/GAMETES-test.csv.gz')
    X_train, X_test, y_train, y_test = train_test_split(data.drop('class', axis=1).values,
                                                        data['class'].values,
                                                        random_state=34895)
...
def test_fit_transform():
    """Make sure RankingFunctions fit_transforms correctly"""
    data = pd.read_csv('data/GAMETES-test.csv.gz')
    X_train, X_test, y_train, y_test = train_test_split(data.drop('class', axis=1).values,
                                                        data['class'].values,
                                                        random_state=34895)
